...
// The CLEAN, CORRECTLY NESTED body content for ChainFund index.html
const cleanFullBody = `  <div class="topbar">
    ...
    <div class="topbar-right">
      ...
      <div id="topbar-user-area" style="display:flex;align-items:center;gap:8px">
        ...
        <button class="topbar-btn btn-signup" id="signup-btn" onclick="switchAuthTab('register');openAuthModal()">Sign Up →</button>
      </div>
    ...
    <div class="sidebar">
      ...
      <div class="sidebar-section" id="section-admin" style="display:none">
        ...
        <div class="nav-item" id="nav-admin" onclick="showPage('admin')">
          <div class="nav-icon">⚙️</div> Admin Panel<span class="nav-badge" id="admin-pending-badge" style="display:none">0</span>
        </div>
      </div>
      <div class="sidebar-divider"></div>
      <div class="sidebar-section">
        <div class="sidebar-section-label">Categories</div>
        <div class="nav-item" onclick="document.getElementById('cat-filter').value='Healthcare';showPage('campaigns');loadCampaigns()">
          <div class="nav-icon">💊</div> Healthcare
        </div>
        <div class="nav-item" onclick="document.getElementById('cat-filter').value='Education';showPage('campaigns');loadCampaigns()">
          <div class="nav-icon">📚</div> Education
        </div>
        <div class="nav-item" onclick="document.getElementById('cat-filter').value='Environment';showPage('campaigns');loadCampaigns()">
          <div class="nav-icon">🌱</div> Environment
        </div>
        <div class="nav-item" onclick="document.getElementById('cat-filter').value='Relief';showPage('campaigns');loadCampaigns()">
      ...
      <div class="sidebar-bottom">
        ...
        <div id="sidebar-connect-btn" style="width:100%">
          <button style="width:100%;padding:11px;border-radius:12px;background:var(--accent-dim);border:1px solid rgba(108,99,255,.3);color:var(--accent);font-family:var(--sans);font-size:13px;font-weight:700;cursor:pointer;transition:all .2s" onclick="connectWallet()">🦊 Connect Wallet</button>
        </div>
      </div>
    </div>
    ...
    <div class="main">
      <div class="page active" id="page-home">
        <div style="padding:0 0 20px">
          <div id="hero-plx" style="background:linear-gradient(135deg,#0a0518 0%,#0d1a40 40%,#051515 100%);border-radius:24px;padding:64px 48px;position:relative;overflow:hidden;margin-bottom:32px;perspective:1000px;transform-style:preserve-3d;box-shadow:0 24px 60px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.05)">
            ...
            <div class="plx-layer" data-speed="0.3" data-mouse="-8" style="position:absolute;top:-80px;right:-50px;width:350px;height:350px;border-radius:50%;background:radial-gradient(circle,rgba(108,99,255,.3),transparent 70%);will-change:transform"></div>
            <div class="plx-layer" data-speed="0.2" data-mouse="5" style="position:absolute;bottom:-60px;left:150px;width:250px;height:250px;border-radius:50%;background:radial-gradient(circle,rgba(0,229,160,.2),transparent 70%);will-change:transform"></div>
            ...
            <div class="plx-layer" id="hero-content" data-speed="-0.1" data-mouse="2" style="position:relative;z-index:1;max-width:650px;will-change:transform;transform-style:preserve-3d">
              ...
              <h1 style="font-family:var(--display);font-size:clamp(3.5rem,7vw,5.5rem);font-weight:900;color:#fff;line-height:0.95;margin-bottom:28px;letter-spacing:-0.03em;text-shadow:0 12px 30px rgba(0,0,0,0.5)"><b>CHAINFUND</b></h1>
              <p style="font-size:16px;color:rgba(255,255,255,.7);line-height:1.7;max-width:480px;margin-bottom:36px;text-shadow:0 2px 8px rgba(0,0,0,0.4)">India's first blockchain fundraising SaaS. Donors verify, in real time, exactly where every rupee went. Smart contracts. Zero middlemen.</p>
              <div style="display:flex;gap:12px;flex-wrap:wrap">
                <button class="btn btn-primary" onclick="showPage('campaigns')" style="padding:16px 32px;font-size:15px;border:1px solid rgba(255,255,255,0.2);box-shadow:0 8px 24px rgba(108,99,255,0.5);transition:all 0.3s">Browse Campaigns →</button>
              </div>
            </div>
          </div>
          ...
          <div class="card" style="margin-bottom:24px">
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--accent);margin-bottom:8px">Process</div>
            <div style="font-family:var(--display);font-size:1.2rem;font-weight:800;color:#fff;margin-bottom:4px">How ChainFund Works</div>
            <div style="font-size:13px;color:var(--text2);margin-bottom:16px">Fully automated — no middlemen, every rupee tracked on-chain.</div>
            <div class="steps-row">
          ...
          <div class="grid3">
            <div class="feat-card tilt-fx"><span class="feat-icon">⛓️</span><h3>Immutable Audit Trail</h3><p>Every transaction permanently recorded. No one can alter or delete a record.</p></div>
            <div class="feat-card tilt-fx"><span class="feat-icon">🤖</span><h3>Smart Contract Escrow</h3><p>Funds held by code. Disbursement only when verifiable milestones are met.</p></div>
            <div class="feat-card tilt-fx"><span class="feat-icon">✅</span><h3>KYC Verified Recipients</h3><p>All recipients identity-verified on-chain. Every disbursement tagged to a verified wallet.</p></div>
            <div class="feat-card tilt-fx"><span class="feat-icon">📊</span><h3>Real-time Dashboards</h3><p>Campaign managers and donors share the same live view from public blockchain nodes.</p></div>
            <div class="feat-card tilt-fx"><span class="feat-icon">🔑</span><h3>Multi-Sig Security</h3><p>Large disbursements need M-of-N trustee approval. No single actor can drain funds.</p></div>
            <div class="feat-card tilt-fx"><span class="feat-icon">↩️</span><h3>Automatic Refunds</h3><p>Miss goal? Smart contract auto-refunds every donor. No forms, no delays.</p></div>
          </div>
        </div>
      </div>
`;
...
const campaignsMarker = content.indexOf('<!-- CAMPAIGNS PAGE -->');

if (bodyStart !== -1 && campaignsMarker !== -1) {
    // Keep everything before <body>, append new body, append everything from CAMPAIGNS PAGE marker onwards
    const newContent = content.slice(0, bodyStart + 6) + '\n' + cleanFullBody + '\n      ' + content.slice(campaignsMarker);
    fs.writeFileSync(path, newContent, 'utf8');
    console.log('Final Structural Restoration Complete.');
} else {
    console.log('Error: Could not find structural anchors.');
}
